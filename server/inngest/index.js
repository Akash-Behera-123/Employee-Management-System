  ...
  async ({ event, step }) => {
    ...
    if (!attendance?.checkOut) {
      ...
      await sendEmail({
        ...
        body: `<div style="max-width: 600px; margin: auto; font-family: Arial, sans-serif; padding: 20px; border: 1px solid #e5e5e5; border-radius: 10px;">
      ...
      //After 1 more hour , make attendance as checked out with status "LATE"
  ...
  async ({ step }) => {
    ...
    if (absentEmployees.length > 0) {
      await step.run(
        ...
        async () => {
          const emailPromise = absentEmployees.map(
            async (emp) => {
              //send email
              await sendEmail({
                to: emp.email,
